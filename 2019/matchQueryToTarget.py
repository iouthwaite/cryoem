...
#5: number of processes to use, can reset the default maximum number by changing the variable "maxprocs" below
...
def compareData(f2,f1,results,start,end,processnum):
	...
	for line in f2:
		...
		if len(line[1]) >= end+1:
			temp = line[1][end]
			for item in f1:
				if len(item[1]) >= start+1:
					if item[1][start] == temp:
...
def comparefiles(file1,file2,start,end,numprocs):
	...
	datachunks = int((len(f1)-len(headerlines)) / (numprocs))
	if datachunks < 1:
		datachunks = 1
	...
	chunks = [f1[x:x+datachunks] for x in xrange(len(headerlines), len(f1), datachunks)]
	...
	for i in range(numprocs):
		...
		if i>(len(chunks)-1):
			break
		p = mp.Process(target = compareData, args=(chunks[i],f1,results,start,end,i))
		jobs.append(p)
		p.start()
...
if __name__ == '__main__':
	...
	print comparefiles(args[0],args[1],int(args[2])-1,int(args[3])-1,int(args[4])) #run the program
